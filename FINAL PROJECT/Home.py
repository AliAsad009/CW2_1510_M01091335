...
if st.session_state.logged_in:
    ...
    with col_a:
        st.page_link("pages/3_Tickets.py", label="Tickets")
    with col_b:
        st.page_link("pages/4_MetaData.py", label="Meta Data")
    with col_c:
        st.page_link("pages/5_Incidents.py", label="Cyber Incidents")
    ...
    if st.button("Create account"):
        if not new_username or not new_password or not confirm_password or not role:
            ...
            if not valid_user:
                ...
                # Always pass role to register_user; enforce role is always saved
