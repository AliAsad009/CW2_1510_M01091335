    ...
    with open(USER_DATA_FILE, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) >= 2 and parts[0] == username:
                ...
                if len(parts) >= 3:
                    new_line = f"{username},{new_hashed},{parts[2]}\n"
                else:
                    new_line = f"{username},{new_hashed}\n"
...
def register_user(username, password, role=None):
    ...
    # Append the new user credentials to the file, including role if provided
    with open(USER_DATA_FILE, "a", encoding="utf-8") as i:
        if role:
            i.write(f"{username},{hashed_str},{role}\n")
        else:
